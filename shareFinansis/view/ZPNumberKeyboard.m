...
- (void)addLines
{
    for (int i = 0; i <4; i ++) {
        ...
        if(i==2){
            VView.frame = CGRectMake(i*KScreen_Width/4, 0, 0.5, keyBoardHeight*3);
        }else{
...
- (void)initCustomKeyborad
{
    for (int x = 0; x < 11; x++)
    {
        ...
        if (x <= 9){
            ...
            //数字0
            NumBtn.tag = 0;
            [NumBtn setFrame:CGRectMake( x%3*(KScreen_Width / 4),  x/3*keyBoardHeight , KScreen_Width / 4 * 2, keyBoardHeight)];
            [NumBtn setTitle:[NSString stringWithFormat:@"%ld",NumBtn.tag] forState:UIControlStateNormal];
        }
...
//    NSLog(@"%ld",tag);
    switch (tag)
    {
        ...
        {
            // 小数点
            if(self.textLabel.text.length > 0 && [self.textLabel.text rangeOfString:@"." options:NSCaseInsensitiveSearch].location == NSNotFound && self.textLabel.text.length < maxLength)
        ...
        case 13:
        {
            ...
            if([self.delegate respondsToSelector:@selector(sureBtnPress:)]){
                [self.delegate sureBtnPress:self];
            }
        }
            break;
        default:
        {
            // 数字
            self.textLabel.text = [NSString stringWithFormat:@"%@%ld",self.textLabel.text,sender.tag];
            //开始输入0时，下一位不是小数点的处理
            NSString *numStr = [NSString stringWithFormat:@"0%ld",(long)sender.tag];
            if ([self.textLabel.text isEqualToString:numStr ]) {
                self.textLabel.text = [NSString stringWithFormat:@"%ld",(long)sender.tag];
            }
            if (self.textLabel.text.length > maxLength) {
                self.textLabel.text = [self.textLabel.text substringToIndex:maxLength];
            }
